...
def write_feedback_sheet(wb, data_rows, year, format_name, total_income=0,
    ...
    if mapping_quality:
        ws.merge_cells(f'A{row}:H{row}')
        _write(ws, row, 1, '3-1. AI 매핑 신뢰도 / 자동 검증 결과', 'section')
        ws.row_dimensions[row].height = 18
        row += 1

        _write(ws, row, 1, '항목', 'header')
        _write(ws, row, 2, '값', 'header')
        ws.merge_cells(f'C{row}:H{row}')
        _write(ws, row, 3, '설명', 'header')
        row += 1

        quality_rows = [
            ('전체 confidence', mapping_quality.get('overall_confidence', '-'), 'AI가 산정한 전체 매핑 확신도'),
            ('헤더 열 수', mapping_quality.get('header_columns', 0), '양식 헤더 행에서 발견한 비어 있지 않은 열 수'),
            ('매핑 열 수', mapping_quality.get('mapped_columns', 0), 'AI 매핑에 포함된 열 수'),
            ('검증 이슈', len(mapping_quality.get('issues', [])), '시트/열/타입/누락 헤더 기준 자동 검증 결과'),
            ('낮은 confidence 열', len(mapping_quality.get('low_confidence', [])), '0.70 미만으로 표시된 컬럼 수'),
        ]
        for label, value, desc in quality_rows:
            style = 'warn' if label in ('검증 이슈', '낮은 confidence 열') and value else 'normal'
            _write(ws, row, 1, label, style)
            _write(ws, row, 2, value, style)
            ws.merge_cells(f'C{row}:H{row}')
            _write(ws, row, 3, desc, style)
            row += 1

        details = []
        for issue in mapping_quality.get('issues', []):
            details.append(('이슈', issue.get('severity', ''), issue.get('message', '')))
            for missing in issue.get('missing_headers', [])[:20]:
                details.append(('누락 헤더', missing.get('letter', ''), missing.get('header', '')))
        for item in mapping_quality.get('low_confidence', []):
            details.append((
                '낮은 confidence',
                f"열{item.get('col')} / {item.get('confidence')}",
                f"{item.get('label', '')} | {item.get('evidence', '')}",
            ))

        if details:
            _write(ws, row, 1, '구분', 'header')
            _write(ws, row, 2, '위치/값', 'header')
            ws.merge_cells(f'C{row}:H{row}')
            _write(ws, row, 3, '내용', 'header')
            row += 1
            for kind, where, msg in details[:60]:
                _write(ws, row, 1, kind, 'warn')
                _write(ws, row, 2, where, 'warn')
                ws.merge_cells(f'C{row}:H{row}')
                _write(ws, row, 3, msg, 'warn')
                row += 1
        else:
            ws.merge_cells(f'A{row}:H{row}')
            _write(ws, row, 1, '자동 검증 기준에서 추가 이슈 없음', 'good')
            row += 1
        row += 1
